---
title: "柯格鐘稅法課程筆記"
description: "個人整理的柯格鐘老師稅法課程筆記。內容未經老師審定，不代表老師、課程或臺大官方立場。"
categories:

  - "課程筆記"
  - "個人筆記"
  - "索引"

tags:

  - "課程筆記"
  - "個人筆記"

keywords:
  - "柯格鐘"
  - "柯格鐘 稅法"
  - "台大柯格鐘"
  - "所得稅法筆記"
  - "租稅法總論筆記"
  - "稅捐規避"
  - "稅法逐字稿"

lang: "zh-Hant"
---

# 柯格鐘稅法課程筆記

<section class="course-home-intro">
  <p class="course-home-kicker">個人上課筆記</p>
  <p class="course-home-lead">這裡放的是我自己聽課、整理、回頭查資料時用的稅法筆記。內容有逐字稿、有課後整理，也有一些後來補上的索引。這不是老師的講義。</p>
  <p>筆記依學期和課程分開。想找某個法條、判決字號或課堂日期，直接用頁首搜尋比較快；想順著課程讀，就從下面的學期欄位進去。</p>
</section>

<section class="course-home-notice">
  <h2>請先看這幾句</h2>
  <p>本站只是個人筆記，未經柯格鐘老師審定，也不代表老師、課程或臺大的官方立場。文字可能有聽寫錯誤、整理失焦、後續理解修正不到位的地方，正式引用、考試或報告請回到課堂資料、法條、判決和老師指定文獻。</p>
  <p>禁止轉抄、整段轉貼、搬運、改名重發，或拿去做商業使用。</p>
</section>

<div class="course-home-grid">
  <section class="course-home-card course-home-card--current">
    <p class="course-home-term">1142</p>
    <h2>1142的課</h2>
    <ul>
      <li><a href="1142/課_二34_所得稅法/">週二 3-4 所得稅法</a></li>
      <li><a href="1142/課_四34_稅法專題/">週四 3-4 稅法專題</a></li>
      <li><a href="1142/課_四89_所得稅法專題研究/">週四 8-9 所得稅法專題研究</a></li>
    </ul>
  </section>

  <section class="course-home-card">
    <p class="course-home-term">1141</p>
    <h2>1141的整理</h2>
    <ul>
      <li><a href="1141/課_二89_租稅法總論/">週二 8-9 租稅法總論</a></li>
      <li><a href="1141/課_四34_所得稅法四/">週四 3-4 所得稅法四</a></li>
      <li><a href="1141/課_四89_稅捐規避專題/">週四 8-9 稅捐規避專題</a></li>
    </ul>
  </section>

  <section class="course-home-card">
    <p class="course-home-term">1121</p>
    <h2>1121的筆記</h2>
    <ul>
      <li><a href="1121/1121_租稅法總論/_1121_租稅法總論_合併/">租稅法總論</a></li>
      <li><a href="1121/1121_租稅規避/">稅捐規避</a></li>
    </ul>
  </section>
</div>

## 快速查找

- 依課程讀：左側導覽有學期和課程。
- 直接找字：頁首搜尋可以搜法條、判決字號、日期和概念。
- 找不到時：換一個老師上課可能用過的關鍵字，例如「基本生活費」「實質課稅」「漏稅罰」。

## 主要主題

- 所得稅法：所得分類、免稅額、扣除額、基本生活費、扣繳、罰鍰。
- 租稅法總論：租稅法律主義、量能課稅、稅捐債務、稽徵程序、行政救濟。
- 稅捐規避：實質課稅、規避行為、不合常規交易、關係企業。
- 稅法專題：納稅者權利保護法、行政程序法、行政訴訟法，以及一些稅務行政事件。
